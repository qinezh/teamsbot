...
import { ConversationReferenceStore } from "../../conversationWithCloudAdapter/interface";
  ...
  public async onTurn(context: TurnContext, next: () => Promise<void>): Promise<void> {
    ...
    switch (type) {
      ...
      case ActivityType.TeamRestored: {
        ...
        await this.conversationReferenceStore.add(getKey(reference), reference, { overwrite: true});
        break;
      }
  ...
  private async tryAddMessagedReference(context: TurnContext): Promise<void> {
    ...
    if (conversationType === "personal" || conversationType === "groupChat") {
      ...
      if (teamId !== undefined) {
        ...
        await this.conversationReferenceStore.add(getKey(teamReference), teamReference, { overwrite: false});
      }
    }
  }
}
